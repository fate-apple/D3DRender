...
static std::unordered_map<AssetHandle, weakRef<composite_mesh>> meshCache; // TODO: Pack flags into key.
static std::mutex mutex;

static ref<composite_mesh> loadMeshFromFileInternal(AssetHandle handle, const fs::path& sceneFilename, uint32 flags)
{
	...
	ref<composite_mesh> result = makeRef<composite_mesh>();
...
{
	AssetHandle handle = getAssetHandleFromPath(sceneFilename.lexically_normal());
