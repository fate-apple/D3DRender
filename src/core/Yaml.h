...
namespace YAML
{
	...
	{
		static Node encode(const AssetHandle& v) { Node n; n = v.id; return n; }
		static bool decode(const Node& n, AssetHandle& h) { h.id = n.as<uint64>(); return true; }
	};
}

#define YAML_LOAD(node, var, name)      \
    {                                   \
        auto nc = node[name];           \
        if(nc){                         \
            var = nc.as<std::remove_reference_t<decltype(var)>>();      \
        }                               \
    }

#define YAML_LOAD_ENUM(node, var, name)         \
    {                                           \
        auto nc = node[name];                   \
        if(nc){                                 \
            var = (std::remove_reference_t<decltype(var)>) (nc.as<int>());      \
        }                                       \
    }                                           \
