...
#ifndef PHYSICS_ONLY
static ref<DxTexture> loadAssimpTexture(const aiScene* scene, const fs::path& sceneFilepath, const std::string& name, uint32 flags = image_load_flags_default)
{
	ref<DxTexture> texture = 0;
...
ref<PbrMaterial> loadAssimpMaterial(const aiScene* scene, const fs::path& sceneFilepath, const aiMaterial* material)
{
	...
	ref<DxTexture> albedo = loadAssimpTexture(scene, sceneFilepath, albedoName);
	ref<DxTexture> normal = loadAssimpTexture(scene, sceneFilepath, normalName, image_load_flags_default | image_load_flags_noncolor);
	ref<DxTexture> roughness = loadAssimpTexture(scene, sceneFilepath, roughnessName, image_load_flags_default | image_load_flags_noncolor);
	ref<DxTexture> metallic = loadAssimpTexture(scene, sceneFilepath, metallicName, image_load_flags_default | image_load_flags_noncolor);

	ref<PbrMaterial> result = makeRef<PbrMaterial>();
