...
static ref<DxTexture> iconsTexture;
...
void renderImGui(DxCommandList* cl)
{
...
static ImTextureID pushTexture(DxCpuDescriptorHandle handle)
{
...
namespace ImGui
{
	...
	void Image(::DxCpuDescriptorHandle& handle, ImVec2 size)
	{
	...
	void Image(::DxCpuDescriptorHandle& handle, uint32 width, uint32 height)
	{
		ImGui::Image(handle, ImVec2((float)width, (float)height));
	}

	void Image(const ref<DxTexture>& texture, ImVec2 size)
	{
		if (size.x == 0)
		{
	...
	void Image(const ref<DxTexture>& texture, uint32 width, uint32 height)
	{
		ImGui::Image(texture, ImVec2((float)width, (float)height));
	}

	bool ImageButton(::DxCpuDescriptorHandle& handle, ImVec2 size, ImVec2 uvTopLeft, ImVec2 uvBottomRight)
	{
		if (numImagesThisFrame < MAX_NUM_IMGUI_IMAGES_PER_FRAME)
		{
	...
	bool ImageButton(::DxCpuDescriptorHandle& handle, uint32 width, uint32 height, ImVec2 uvTopLeft, ImVec2 uvBottomRight)
	{
		return ImGui::ImageButton(handle, ImVec2((float)width, (float)height), uvTopLeft, uvBottomRight);
	}

	bool ImageButton(const ref<DxTexture>& texture, ImVec2 size, ImVec2 uvTopLeft, ImVec2 uvBottomRight)
	{
		return ImGui::ImageButton(texture->defaultSRV, size, uvTopLeft, uvBottomRight);
	}

	bool ImageButton(const ref<DxTexture>& texture, uint32 width, uint32 height, ImVec2 uvTopLeft, ImVec2 uvBottomRight)
	{
	...
	bool AssetHandleInternal(const char* label, const char* type, AssetHandle& asset)
	{
		char buffer[32] = "";
		if (asset)
		{
			snprintf(buffer, sizeof(buffer), "%llu", asset.id);
		}
	...
	bool PropertyAssetHandle(const char* label, const char* type, AssetHandle& asset)
	{
		pre(label);
		bool result = AssetHandleInternal("", type, asset);
		post();
		return result;
	}
